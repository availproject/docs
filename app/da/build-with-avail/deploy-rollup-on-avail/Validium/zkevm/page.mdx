---
image: "/img/docs-link-preview.png"
title: Polygon zkEVM
asIndexPage: true
---

import { Cards } from 'nextra/components'
import { FileIcon, LinkIcon } from '@components/icons'

# Polygon zkEVM

<Cards>
  <Cards.Card icon={<FileIcon />} title="Overview" href="/da/build-with-avail/deploy-rollup-on-avail/Validium/zkevm/overview" />
  <Cards.Card icon={<FileIcon />} title="Avail-Powered zkEVM Validium" href="/da/build-with-avail/deploy-rollup-on-avail/Validium/zkevm/zkevm" />
  <Cards.Card icon={<LinkIcon />} title="Validium Node" href="https://github.com/availproject/validium-node" />
  <Cards.Card icon={<LinkIcon />} title="Validium Contracts" href="https://github.com/availproject/validium-contracts" />
  <Cards.Card icon={<LinkIcon />} title="Validium Bridge" href="https://github.com/availproject/validium-bridge-service" />
</Cards>
