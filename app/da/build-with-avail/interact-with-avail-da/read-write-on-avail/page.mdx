...
1. [`Get Testnet Tokens`](/da/build-with-avail/interact-with-avail-da/faucet): To help you get set with some `AVAIL` tokens on the Turing testnet.
2. [`Create an AppID`](/da/build-with-avail/interact-with-avail-da/app-id): The guide below focusses on reading & submitting data on a particular AppID. It will help
...
To set up a dev environment for `avail-js`, please follow the steps [outlined here](/da/api-reference/avail-node-api#setting-up-the-dev-environment).
...
To check out how to create an AppID on Avail, [refer to this page](/da/build-with-avail/interact-with-avail-da/app-id).
</Callout>
