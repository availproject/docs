...
<img src="/img/avail-nexus/payment-example.png" width="100%" height="100%" style={{ borderRadius: "1%" }} />
 ...
 {/*  ## Token Transfer Mailbox

 This is one of the few examples that Avail team has built to help developers imagine the power of Nexus and what can be built on top of it. */}
...
Now that the proof is fetched, the `ZKSync 2` chain can verify any message or a trasaction by retrieving the **message** from mailbox and verifying the proof by creating a new instance of `ZKSyncVerifier` called `zksyncAdapter`
...
```
5. We create a instnace of the `ZKSyncVerifier` as `zksyncAdapter`, and defines the expected `PaymentReceipt`.
```typescript
 const zksyncAdapter = new ZKSyncVerifier({
      [app_id]: {
