...
**Note**: export ETH_RPC_URL="$(kurtosis port print cdk cdk-erigon-node-001 rpc)” in this command the right service name is `cdk-erigon-sequencer-001`
...
In order to deploy on Sepolia, you need to change the [salt](https://github.com/availproject/kurtosis-cdk/blob/avail-develop-v0.2.29/templates/contract-deploy/deploy_parameters.json#L12) value to avoid a deployment failure. This can be done easily by running this script:
