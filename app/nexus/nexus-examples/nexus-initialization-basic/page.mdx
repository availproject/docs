...
You can refer to this [page in our docs](/nexus/concepts/bridge-v-swap) for more details.
</Callout>
...
and `.bridge()` functions *(We'll go through them in the following tutorials)*. You can also fetch unified multichain balances using the SDK.
...
- The flows to do this differ between Nexus SDK and Nexus Elements.
...
## Initialize the Nexus SDK using Next JS (A minimal example using nexus)
...
2. The second button will be used to initialize the SDK using `nexus`.
...
4. `getUnifiedBalances()`: This function calls `sdk.getBalancesForBridge()` from the Nexus SDK to fetch the unified balance of the user.
...
This component will be used to initialize the SDK using `nexus`. 
<Callout type="info">
...
```
<Callout type="info">
`sdk.getBalancesForBridge()` returns a JSON object with the balances of the user on each of the supported chains.
We're rendering it out on the screen raw to keep the tutorial simple, but you can always format it as you please.

[Read more about Bridge Balances](/nexus/concepts/bridge-v-swap)
</Callout>
