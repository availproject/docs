...
In this section we will create a minimal Vite project that uses wagmi to connect a wallet to the app and initialize the Nexus SDK using `nexus-widgets`.

### Set up a basic Vite project

1. Navigate into a directory of your choice and create a new Vite project at it's root:

```bash filename="Terminal"
pnpm create vite@latest . --template react-ts
```

2. Add the following dependencies to your `package.json`, including `nexus-widgets` and `wagmi`:

```json filename="package.json"
{
  "name": "nexus-vite-template",
    "private": true,
    "version": "0.0.0",
    "type": "module",
    "scripts": {
...
4. Update the `src/App.tsx` file to the following:
...
5. Run a dev server using:

```bash filename="Terminal"
pnpm dev
```

You should now have a bare bones Vite project running in your browser.
