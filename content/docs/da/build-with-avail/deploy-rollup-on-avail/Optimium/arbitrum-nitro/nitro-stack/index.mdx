...
This guide follows the deployment of an Arbitrum Orbit chain (L3) with Arbitrum Sepolia (L2) as the base chain. You can follow the same steps with any [other supported chains](/docs/da/build-with-avail/deploy-rollup-on-avail/Optimium/arbitrum-nitro/nitro-stack#reference) with the relevant contract addresses.
...
- An application key (`app-id`) for your rollup. Learn [how to get your `app-id` here](/docs/da/build-with-avail/interact-with-avail-da/app-id). 
