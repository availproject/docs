---
title: "Proxy Accounts on Avail DA"
description: "Delegate specific account permissions to proxy accounts for secure and efficient management on Avail DA."
...
<Tabs defaultValue="transfer-avail-via-proxy">
  <TabsList>
    <TabsTrigger value="transfer-avail-via-proxy">Transfer `AVAIL` via proxy</TabsTrigger>
    <TabsTrigger value="stake-avail-via-proxy">Stake `AVAIL` via proxy</TabsTrigger>
  </TabsList>
  <TabsContent value="transfer-avail-via-proxy">
...
pay the gas fee for it, no funds from the main account will actually be transferred.
  ...
  <TabsContent value="stake-avail-via-proxy">
...
Do not use proxy calls for Avail-to-Ethereum bridge sends. Bridge sends must be submitted directly with `vector_sendMessage`; wrapped bridge sends are rejected.
</Callout>
