...
An intent represents a user's request to execute a crosschain transaction ([Nexus operation](/docs/nexus/concepts/nexus-ops)) when they lack sufficient funds or liquidity on the destination blockchain. 
...
The intent is verified, and taken up by a [solver](/docs/nexus/concepts/solvers) who provides liquidity and fills the intent (request for funds) on the user's preferred destination chain.
...
This is an example of what an intent looks like when simulating a `Bridge` operation: 
```jsx
type ReadableIntent = {
  allSources: {
    amount: string;
