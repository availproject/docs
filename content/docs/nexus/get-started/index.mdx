---
image: "/img/docs-link-preview.png"
title: Get started
---
 Avail Nexus is a meta-interoperability protocol that unifies users, assets, apps, and chains, letting developers build once, scale everywhere, and access liquidity and users from across the Web3 economy.



- Unifies assets across chains
- Executes intents across ecosystems
- Let's apps interact with contracts anywhere
- Handles gas, routing, and verification under the hood
- Treats liquidity as a single global resource

Deposit, Swap, Stake, Buy, Play - Avail Nexus abstracts chains away so that "intent" replaces complexity.

A seamless “bridgeless" experience. 

## Integrate Nexus - Create Liquid Apps

Nexus aims to reframe the mindset of both consumers and developers when using or building apps. With Nexus applications instantly gain access to users and liquidity from across Web3. They are longer just simple apps, they become *Liquid Apps*.

Get started building your Liquid App today by integrating Nexus.

<IconCardGrid>
  <IconCard
    icon={<CodeIcon />}
    title="Nexus SDK (Headless)"
    description="Integrate Nexus directly into your app logic with full control over the UX — bridging, swaps, and intents via a simple TypeScript API."
    href="/docs/nexus/nexus-quickstart/nexus-core"
  />
  <IconCard
    icon={<Puzzle />}
    title="Nexus UI Elements (UI Widgets)"
    description="Drop in pre-built UI components for cross-chain operations — get bridging and swaps working in minutes with minimal code."
    href="https://elements.nexus.availproject.org/docs/get-started"
    external
  />
</IconCardGrid>

## Learn about Nexus Features
Dive deep into the core features your application can leverage when building with Avail. 

- Bridge, Transfer, Bridge & Execute - Bridge Methods
- Exact In & Exact Out crosschain swaps - Swap Methods

*Build liquid apps that can provide a truly bridgeless experience onchain.*


<IconCardGrid>
  <IconCard
    icon={<Bridge />}
    title="Bridge, Transfer, Bridge & Execute"
    description="Cross-chain bridging methods — move assets between chains, transfer to different addresses, or bridge and execute a contract call in one step."
    href="/docs/nexus/concepts/nexus-ops"
  />
  <IconCard
    icon={<Swap />}
    title="Crosschain Swaps"
    description="Exact In and Exact Out crosschain swaps — exchange tokens across chains with precise control over input or output amounts."
    href="/docs/nexus/concepts/xcs-swaps"
  />
</IconCardGrid>

### Quick access Nexus SDK reference

<IconCardGrid>
  <IconCard
    icon={<Bridge />}
    title="Bridge Methods"
    href="/docs/nexus/nexus-sdk/bridge-methods"
  />
  <IconCard
    icon={<Swap />}
    title="Swap Methods"
    href="/docs/nexus/nexus-sdk/swap-methods"
  />
</IconCardGrid>
