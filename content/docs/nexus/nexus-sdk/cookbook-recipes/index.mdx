...
```typescript
import { sdk } from '@avail-project/nexus-core';
...
```
<br/>
The response structure for the above example can be found [here](/docs/nexus/nexus-sdk/reference/bridge-methods/bridge).
...
<Accordions type="single">
  <Accordion title="Sample CHAIN_METADATA structure">
  ```json
  {
    "421614": {
      "id": 421614,
...
<Accordions type="single">
  <Accordion title="Sample STEPS_LIST event output">
  ```json
  {
    "name": "STEPS_LIST",
    "args": [
...
<Accordions type="single">
  <Accordion title="Sample STEP_COMPLETE event output">
  ```json
  {
    "name": "STEP_COMPLETE",
    "args": {
...
<Accordions type="single">
  <Accordion title="Swap step types in order">
  ```
  SWAP_START
  DETERMINING_SWAP
