...
description: "Events emitted by bridge(), bridgeAndTransfer(), and bridgeAndExecute() — track intent steps and completion."
---
...
- Use a callback function to listen to the `NEXUS_EVENTS.STEPS_LIST` event to get the steps list for the bridge operation.
...
<Cards num={1}>
    <Card icon={<GithubIcon24 className="svg-inherit" />} title="Bridge steps" href="https://github.com/availproject/nexus-sdk/blob/develop/src/commons/types/bridge-steps.ts" target="_blank" rel="noopener noreferrer" arrow />
</Cards>
