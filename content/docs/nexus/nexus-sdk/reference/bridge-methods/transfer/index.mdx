...
- `options`: `OnEventParam` (optional): Optional callback function to listen to the events emitted by the transfer operation.
 ...
 <Callout>
You can check out the type definitions for each step type in the [Bridge Events](/docs/nexus/nexus-sdk/reference/bridge-methods/bridge-events) page.
</Callout>

## Example

```typescript title="TypeScript"
import { TransferParams, TransferResult, NEXUS_EVENTS } from '@avail-project/nexus-core';

// Smart transfer with automatic optimization
const bridgeAndTransferResult = await sdk.bridgeAndTransfer(
  {
    token: 'USDC',
    amount: 1_530_000n, // 1.53 USDC (6 decimals)
