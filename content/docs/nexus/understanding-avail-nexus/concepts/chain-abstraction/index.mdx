...
Instead of converting funds first, users specify a [clear intent](/docs/nexus/understanding-avail-nexus/concepts/intents-and-solvers). The protocol collects funds from source chains, shows intent and fees for confirmation, then publishes the intent to solvers.

[Solvers](/docs/nexus/understanding-avail-nexus/concepts/intents-and-solvers) compete to provide liquidity on the destination chain, and once available, users confirm the transaction. Settlement with solvers happens asynchronously using the collected funds.

As part of setup, devs configure [allowances](/docs/nexus/understanding-avail-nexus/concepts/allowances) per chain and token. Users approve or reject these allowances. For supported chains and tokens, the protocol uses these allowances to deposit required funds for transactions on the destination chain.
