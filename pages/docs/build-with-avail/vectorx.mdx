...
VectorX is the implementation of Avail's the Data attestation bridge that is used to bridge
data from Avail to the EVM compatible chains in form of data roots commitments. It is implemented as a
zero-knowledge proof circuits in SP1. SP1 is the most feature completed zero-knowledge virtual machine
...
Once the transaction is included in an Avail block and block finalized a `data root` is generated for this block, and bridged to
Ethereum. Bridging to avail network is done every 360 Avail blocks and the commitment included in the VectorX contract.
...
When the data root is bridged to the Ethereum network, it is possible to query for the inclusion proof(Merkle proof) using the `bridge-api` service
deployed for Turing testnet `https://turing-bridge-api.fra.avail.so` and mainnet `https://bridge-api.avail.so`.
Bridge API is a service that provides all necessary endpoints and allows easy integration of the bridge.
