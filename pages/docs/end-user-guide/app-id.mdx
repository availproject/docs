...
## But how to actually generate my own `AppId`?

1. Simply [click this link](https://explorer.avail.so/#/extrinsics) OR navigate to the `extrinsics` section of the explorer through the `developer` tab.

2. Select the `dataAvailability` pallet, and the `createApplicationKey` method.
...
3. Make sure you have one or more Avail DA wallets connected to the explorer. If you don't know how to do so,
...
- This does not constitute an attack vector since that any app or execution layer building on top of Avail DA can always pick and choose which 
data they want to pick up from Avail DA. You can ignore the rest :)