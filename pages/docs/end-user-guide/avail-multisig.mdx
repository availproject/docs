...
<Tabs.Tab>
1. Go to the [`addresses`](https://explorer.avail.so/#/addresses) page of the explorer.
...
Go to the [accounts page of the avail-apps explorer](https://explorer.avail.so/#/accounts), and click on `Multisig`. This will give you a UI interface
...
Due to the way the `multisig` pallet works on Avail DA, a multisig derived from the same set of addresses
will always have the same address.
Thus, all signatories can simply create a multisig on their machine with addresses of all the other signatories to derive the same '*multisig address*'.
