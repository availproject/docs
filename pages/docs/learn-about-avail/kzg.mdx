...
A special property of KZG commitment is that if, let's say, a polynomial \( P_1 \) is constructed using \( N \) points, and if those \( N \) points are erasure coded, the resulting points also lie on the same polynomial. This property of KZG is exploited to create a linkability between the commitment and erasure coding done, hence giving a cryptographic guarantee that erasure coding itself was done correctly. This property makes Avail a validity proof-based design.
...
The plain KZG is not ZK. For example, $com_f = g^{f(\tau)}$ is deterministic.
...
- Evaluate: (where $r$ and $r'$ are both randomizers)
  $f(x) + ry - f(u) = (x - u)(q(x) + r'y) + y(r - r'(\tau - u))$
