...
<br/>
<p align="center">
<img src="/img/kzg.png" width="70%" height="70%"/>
</p>
...
- $\textit{gp} = (g, g^{\tau}, g^{\tau^2}, \ldots, g^{\tau^d})$
- delete $\tau$ (trusted setup)

#### Step 2: Commit

- $\text{commit} (\textit{gp}, f) \rightarrow \text{com}_f$
...
- $\text{eval}(\textit{gp}, f, u) \rightarrow v, \pi$: where $u$ is a random point the verifier chooses.
  - $f(x) - f(u) = (x - u) q(x)$ as $u$ is a root of $f(x) - f(u)$
  - Compute $q(x)$ and $\pi = g^{q(\tau)}$ computed using $\textit{gp}$ elements.
