...
Every extrinsic (transaction) executed on Avail costs a certain amount of `AVAIL` tokens. This fees are vary for different
...
3. **Length fee**: This fee is proportional to the amount of storage required to store the resultant transaction on-chain.

4. **Congestion fee**: Default value `1`. This is a multiplier that is applied to the `Weight fee` only when
