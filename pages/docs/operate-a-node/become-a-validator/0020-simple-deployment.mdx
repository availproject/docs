...
1. First, create a project and name it appropriately.
<br/>
...
3. For the type, SharedvCPU and CX21 (or anything stronger) will do the trick.
<br/>
<img src="/img/hetzner/hetzner_type.png" width="100%" height="100%"/>

4. Make sure that you have entered your SSH keys.
<br/>
<img src="/img/hetzner/hetzner_ssh_keys.png" width="100%" height="100%"/>

5. Finally, give it a good name.
<br/>
<img src="/img/hetzner/hetzner_name.png" width="500px" height="100%"/>

6. With the server created, you can copy the public IP and SSH in.
<br/>
...
7. Hopefully, you are greeted with the welcome message.
<br/>
...
The **MINIMUM** version for Turing is `2.1.4`.
</Callout>
...
sudo docker run --restart=on-failure -d -v /root/avail/node-data:/da/node-data -p 9944:9944 -p 30333:30333 docker.io/availj/avail:v1.8.0.3 --chain turing -d /da/node-data --validator --name MyAwesomeContainerNode

# Option 2: If you are using Docker on SELinux use this script
docker run --restart=on-failure -d -v /root/avail/node-data:/da/node-data:z -p 9944 -p 30333 docker.io/availj/avail:v1.8.0.3 --chain turing -d /da/node-data --validator --name MyAwesomeContainerNode

# Option 3: If you are using Podman use this script
podman run -d -v /root/avail/node-data:/da/node-data -p 9944 -p 30333 docker.io/availj/avail:v1.8.0.3 --chain turing -d /da/node-data --validator --name MyAwesomeContainerNode

# Option 4: If you are using Podman on SELinux use this script
podman run -d -v /root/avail/node-data:/da/node-data:z -p 9944 -p 30333 docker.io/availj/avail:v1.8.0.3 --chain turing -d /da/node-data --validator --name MyAwesomeContainerNode
```
