...
4. Let's create a file on `/etc/systemd/system/` and name it `avail.service`. If you are using a non-root user, you will need to execute this operation using the `sudo` command.
...
6. Paste the following text and save it:
...
7. Now, let's enable the service file and start our deamon
...
8. To check for logs, we can run the following command:
