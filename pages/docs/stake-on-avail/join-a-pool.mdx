...
To join a nomination pools navigate to the [<ins>Pools Page</ins>](https://explorer.avail.so/#/staking/pools). Select a pool to join where the existing members are less than the max members per pool. Click on `+Join` to enter a pool.
<br/>
<img src="/img/nominating/pool-01.png" width="100%" height="100%" />


### 2. Select Wallet & Stake Amount
Select the wallet you want to join with, also the amount of AVAIL to stake with. Then click on `+Join`
<br/>
