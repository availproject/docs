...
In Optimiums, users can execute transactions and interact with the system seamlessly. Fraud proofs serve as the cornerstone for ensuring that all operations within the network are legitimate and under the established rules. Withdrawals are processed by submitting Merkle proof to a proposed state root that proves the withdrawal transaction's inclusion on the L2 chain.
