...
6. **Implications of Data Unavailability**:
   - **Data Withholding Attack**: Malicious proposer can propose an invalid state (ex. with all the assets on an address controlled by itself), which cannot be challanged because transaction data is required with current fraud proofs implementations. This means that they can steal all the funds inside validating bridge.
...
5. **Implications of Data Unavailability**:
   ...
   - **Data Withholding Attack**: Malicious sequencer can withhold the data needed to reconstruct the state and effectively hold the funds in the validating bridge as a ransom. Invalid state cannot be proposed, as a proof of validity is required.
