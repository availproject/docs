...
function getPromptUrl(baseURL: string, url: string) {
  return `${baseURL}?q=${encodeURIComponent(
    `I’m looking at this Avail Nexus UI Elements documentation: ${url}.
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
  ...
  const handleCopyPage = () => {
    ...
    trackEvent("ai_copy_page_clicked", {
      content_length: page.length,
    });
  };

  const handleAIServiceClick = (service: "v0" | "chatgpt" | "claude") => {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="shadow-none">
            {menuItemConfigs.map((config) => (
              <DropdownMenuItem key={config.id} asChild>
                <a
                  href={getPromptUrl(config.baseUrl, url)}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => handleAIServiceClick(config.id)}
                >
                  {config.icon}
                  {config.id === "v0" ? (
                    <span className="-translate-x-0.5">{config.label}</span>
                  ) : (
                    config.label
                  )}
                </a>
